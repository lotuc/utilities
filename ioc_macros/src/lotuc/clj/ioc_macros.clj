...
;; https://github.com/clojure/core.async/blob/aa6b951301fbdcf5a13cdaaecb4b1a908dc8a978/src/main/clojure/clojure/core/async/impl/ioc_macros.clj

(ns ^{:skip-wiki true}
 lotuc.clj.ioc-macros
...
(defmethod -item-to-ssa :case
  ...
  (gen-plan
   [end-blk (add-block)
    ...
    _case-id (add-instruction (->Case val-id (map (comp :form :test) tests)
                                      clause-blocks
                                      default-block))
    _ (set-block end-blk)
    ret-id (add-instruction (->Const ::value))]
...
(defmethod -item-to-ssa :transition
  ...
  (gen-plan
   [blk (add-block)
    ...
    _val (add-instruction (->CustomTerminator name blk vals (meta form)))
    _ (set-block blk)
...
(defn- emit-state-machine [machine num-user-params custom-terminators]
  (let [index (index-state-machine machine)
        ...
        _block-catches (:block-catches machine)]
    `(fn state-machine#
...
(defn analyze [body [crossing-env env] user-transitions]
...
(defn state-machine [body num-user-params [_crossing-env _env :as e] user-transitions]
  (-> (analyze body e user-transitions)
